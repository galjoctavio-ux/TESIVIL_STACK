...
export const crearRegistroRevision = async (payload, tecnicoAuth) => {
  ...
  if (sePuedeApagar) {
    // Si se apaga todo, usamos el valor de fuga directa (ingresado por el técnico)
  ...
  equiposCalculados.forEach(eq => {
    if (eq.estado_equipo === 'Malo') {
      kwhIneficienciaEquipos += (eq.kwh_bimestre_calculado || 0) * 0.40;
    } else if (eq.estado_equipo === 'Regular') {
      kwhIneficienciaEquipos += (eq.kwh_bimestre_calculado || 0) * 0.20;
    }
  });
